...
if (isset($data['id'])) {
    ...
    if ($participantId > 0) {
        ...
        $sql = "DELETE FROM `eform-inscription-formation-specifique` WHERE id = ?";
        ...
        if ($result > 0) {
            ...
            echo json_encode([
                'status' => 'success',
                'message' => 'Le catalogue a été supprimé avec succès.'
            ]);
        } else {
            // If no rows were affected, send an error response
            echo json_encode([
                'status' => 'error',
                'message' => 'Aucun catalogue trouvé avec cet identifiant.'
            ]);
        }
    } else {
